...
public class KeyStoreLoader {
    ...
    public KeyStoreLoader load(Path baseDir, String certFileName) throws Exception {
        ...
        Path serverKeyStore = baseDir.resolve(certFileName);
        ...
        if (!Files.exists(serverKeyStore)) {
            ...
            // 中间所设置的证书属性可以自行修改。
            SelfSignedCertificateBuilder builder = new SelfSignedCertificateBuilder(keyPair).setCommonName("UaClient@Jellyleo").setOrganization("JL").setOrganizationalUnit("per").setLocalityName("jl").setStateName("JiangSu").setCountryCode("CN").setApplicationUri("urn:Jellyleo:UnifiedAutomation:UaExpert@Jellyleo").addDnsName("Jellyleo").addIpAddress("127.0.0.1");
