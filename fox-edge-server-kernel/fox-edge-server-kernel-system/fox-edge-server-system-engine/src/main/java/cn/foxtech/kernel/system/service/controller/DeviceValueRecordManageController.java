...
import cn.foxtech.common.entity.service.devicevaluerecord.DeviceValueRecordEntityMapper;
import cn.foxtech.common.entity.service.devicevaluerecord.DeviceValueRecordMaker;
...
@RestController
@RequestMapping("/device/value/record")
public class DeviceValueRecordManageController {
    @Autowired
    ...
    public AjaxResult selectEntityListPage(String filter, String order, long pageNmu, long pageSize) {
        try {
            // 查询总数
            String selectCount = PageUtils.makeSelectCountSQL("tb_device_value_record", filter);
            Integer total = this.mapper.executeSelectCount(selectCount);

            // 分页查询数据
            String selectPage = PageUtils.makeSelectSQLPage("tb_device_value_record", filter, order, total, pageNmu, pageSize);
            List<DeviceValueRecordPo> poList = this.mapper.executeSelectData(selectPage);

            List<DeviceValueRecordEntity> entityList = DeviceValueRecordMaker.makePoList2EntityList(poList);
