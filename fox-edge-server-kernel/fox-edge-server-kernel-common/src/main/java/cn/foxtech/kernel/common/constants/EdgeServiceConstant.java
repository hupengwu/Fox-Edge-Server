/* ----------------------------------------------------------------------------
 * Copyright (c) Guangzhou Fox-Tech Co., Ltd. 2020-2024. All rights reserved.
 * --------------------------------------------------------------------------- */

package cn.foxtech.kernel.common.constants;

public class EdgeServiceConstant {
    public static final String field_cpu_id = "cpuId";
    public static final String field_env_type = "envType";
    public static final String value_env_type_device = "device";
    public static final String value_env_type_docker = "docker";

    public static final String field_work_mode = "workMode";
    public static final String value_work_mode_local = "local";
    public static final String value_work_mode_cloud = "cloud";
}
