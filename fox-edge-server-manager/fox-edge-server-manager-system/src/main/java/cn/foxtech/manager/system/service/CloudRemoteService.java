...
public class CloudRemoteService {
    ...
    public synchronized void login() throws IOException {
        // 获得账号/密码
        ConfigEntity configEntity = this.manageService.getConfigEntity(this.foxServiceName, this.foxServiceType, RepositoryConfigConstant.filed_config_name);
        if (configEntity == null) {
            throw new RuntimeException("登录云端服务器：从Redis配置中获得账号/密码失败！");
        }

        // 取出信息
        this.host = (String) configEntity.getConfigValue().getOrDefault(RepositoryConfigConstant.filed_config_host, "http://localhost:8080");
        this.username = (String) configEntity.getConfigValue().getOrDefault(RepositoryConfigConstant.filed_config_username, "username");
        this.password = (String) configEntity.getConfigValue().getOrDefault(RepositoryConfigConstant.filed_config_password, "");
        this.lockdown = (Integer) configEntity.getConfigValue().getOrDefault(RepositoryConfigConstant.filed_config_lockdown, 60);
