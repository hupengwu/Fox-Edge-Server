...
if [ -z ${app_type:1:1} ]; then 
	echo "读取$app_home/shell/$model_name/service.conf的appType配置参数失败！"
	exit
fi
if [ -z ${app_name:1:1} ]; then 
	echo "读取$app_home/shell/$model_name/service.conf的appName配置参数失败！"
	exit
fi
...
if [[ $app_engine == java ]]; then
	if [ -z ${jar_name:1:1} ]; then 
		echo "读取$app_home/shell/$model_name/service.conf的jarName配置参数失败！"
		exit
	fi
