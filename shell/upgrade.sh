...
if [ -z ${component:1:1} ]; then 
	echo "缺少component参数！范例：./upgrade.sh kernel manager-service 1.0.0 master 9000"
	exit
fi

if [ -z ${model_name:1:1} ]; then 
	echo "缺少model_name参数！范例：./upgrade.sh kernel manager-service 1.0.0 master 9000"
	exit
fi

if [ -z ${version:1:1} ]; then 
	echo "缺少version参数！范例：./upgrade.sh kernel manager-service 1.0.0 master 9000"
	exit
fi

if [ -z ${stage:1:1} ]; then 
	echo "缺少stage参数！范例：./upgrade.sh kernel manager-service 1.0.0 master 9000"
	exit
fi

if [ -z ${server_port:1:1} ]; then 
	echo "缺少server_port参数！范例：./upgrade.sh kernel manager-service 1.0.0 master 9000"
	exit
fi
...
source $app_home/repository/service/$model_name/$version/$stage/$component/tar/shell/$component/$model_name/service.conf
...
if [[ -n ${loader_name:1:1} ]]; then 
	cp -rf $app_home/repository/service/$model_name/$version/$stage/$component/tar/bin/$component/$model_name/$loader_name $app_home/bin/$component/$model_name
...
#复制conf目录
cp -rf $app_home/repository/service/$model_name/$version/$stage/$component/tar/conf $app_home
