...
public class RedisTopicProxyService {
    ...
    private static final String channel_head = "/service/proxy-redis-topic/proxy/redis/topic/channel/";
    private static final String device_head = "/service/proxy-redis-topic/proxy/redis/topic/device/";
    ...
    public String executeChannel(String url, String body, RequestMethod method) throws InterruptedException, IOException {
        ...
        // 等待消息的到达：根据动态key
        String respond = (String) SyncFlagObjectMap.inst().waitDynamic(key, timeout + extra_timeout_channel);
        if (respond == null) {
    ...
    public String executeDevice(String url, String body, RequestMethod method) throws InterruptedException, IOException {
        ...
        if ((RedisTopicConstant.topic_device_request + RedisTopicConstant.model_public).equals(topicRequest)) {
            request.put(DeviceMethodVOFieldConstant.field_client_name, RedisTopicConstant.model_proxy4http2topic);
        }
