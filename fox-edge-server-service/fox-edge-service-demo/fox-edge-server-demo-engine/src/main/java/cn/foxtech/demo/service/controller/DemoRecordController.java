...
public class DemoRecordController {
    ...
    private void extend(List<Map<String, Object>> list) {
        ...
        for (Map<String, Object> row : list) {
            DemoTaskEntity periodTaskEntity = this.entityManageService.getEntity((Long) row.get("taskId"), DemoTaskEntity.class);
    ...
    public AjaxResult selectEntityListPage(String filter, String order, long pageNmu, long pageSize) {
        try {
            // 查询总数
            String selectCount = PageUtils.makeSelectCountSQL("tb_service_demo_record", filter);
            Integer total = this.mapper.executeSelectCount(selectCount);

            // 分页查询数据
            String selectPage = PageUtils.makeSelectSQLPage("tb_service_demo_record", filter, order, total, pageNmu, pageSize);
            List<DemoRecordEntity> poList = this.mapper.executeSelectData(selectPage);
            for (DemoRecordEntity po : poList) {
