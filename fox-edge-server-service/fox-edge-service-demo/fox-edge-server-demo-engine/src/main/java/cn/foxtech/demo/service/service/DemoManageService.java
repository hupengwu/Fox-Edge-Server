...
import cn.foxtech.demo.service.mapper.demotask.DemoTaskEntityService;
 ...
 */
@Component
public class DemoManageService extends ServiceEntityManageService {
    @Autowired
    protected DemoTaskEntityService demoTaskEntityService;
    ...
    public void instance() {
        ...
        // 注册数据库
        dBService.put(DemoTaskEntity.class.getSimpleName(), this.demoTaskEntityService);

        // 注册生产者
        producer.add(DemoTaskEntity.class.getSimpleName());

        // 告知：生产者如何装载数据源
        this.getSourceMySQL().add(DemoTaskEntity.class.getSimpleName());
        ...
        // 数据的发布模式
        this.entityPublishManager.setPublishEntityUpdateTime(DemoTaskEntity.class.getSimpleName(), EntityPublishConstant.value_mode_config, EntityPublishConstant.value_type_cache, DemoTaskEntity.class.getSimpleName());
        this.entityPublishManager.setPublishEntityUpdateTime(DemoRecordEntity.class.getSimpleName(), EntityPublishConstant.value_mode_record, EntityPublishConstant.value_type_mysql, "tb_period_record");

        // 指明允许借manage服务的option，对数据库表进行option的查询操作
        this.entityOptionManager.setOptionEntity(DemoTaskEntity.class.getSimpleName(), "tb_period_task", new String[]{"task_name", "device_type"});
        this.entityOptionManager.setOptionEntity(DemoRecordEntity.class.getSimpleName(), "tb_period_record", new String[]{"task_id", "record_batch", "device_id", "object_name"});
    }
}
